...
<head>
	...
	<link rel="stylesheet" href="BlueColor.css">
				<title>Blue School - Meus projetos</title>		
...
<body  oncontextmenu="return false" onload="myFunction()" style="margin:0;">
	...
	<div class="container" id="blue-text-separador">Meus Projetos</div><br>

	<div class="container" id="blue-search">
	<form class="example"  method="POST" action="SearchProjetosGestor.php">
	...
	?> 
		...
		<div id="blue-title-front-card">Projeto
		</div><br>
		<p id="blue-text-flip-card">Projeto: <?php echo $dado['ID']; ?></p>
		<p id="blue-text-flip-card">Nome: <?php echo $dado['NOME']; ?></p>
		<p id="blue-text-flip-card">Descrição: <?php echo $dado['DESCRICAO'];?></p>
		</div>
		<!-- Fim flip-card-front -->
		...
		<br><br>
		<p id="blue-text-flip-card">Solicitante: <?php echo $dado['SOLICITANTE']; ?> </p> 
		<p id="blue-text-flip-card">Unidade: <?php echo $dado['UNIDADE']; ?></p>
		<p id="blue-text-flip-card">Registrado:<input id="blue-flip-card-date" type="date" value="<?php echo $dado['DIA'];?>"/></p> 
		</div>
		<!-- Fim flip-card-back -->
		
		</div>
		<div id="blue-optios-config-dados" onclick="window.location.href='ConfigurarProjetoGestor.php?id=<?php echo $dado['ID'];?>';">Configurar
